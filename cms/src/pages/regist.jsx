    ...
    const [dataRegist, setRegist] = useState({
      ...
      const addSubmit = async (e) => {
        ...
        try {
          ...
          let response = await axios.post('https://anasendiri.cloud/regist',dataRegist,{
              headers:{
   ...
   <div className="contain py-16">
        <div className="max-w-lg mx-auto shadow px-6 py-7 rounded overflow-hidden">
            ...
            <p className="text-gray-600 mb-6 text-sm">
                Register for new staff
            </p>
            <form action="#" method="post" autoComplete='off' onSubmit={addSubmit}>
                <div className="space-y-2">
                    <div>
                        <label for="email" className="text-gray-600 mb-2 block">Email Address</label>
                        <input onChange={inputChange} type="text" name="email" id="email"
                    ...
                    <div>
                        <label for="phoneNumber" className="text-gray-600 mb-2 block">Phone Number</label>
                        <input type="text" name="phoneNumber" id="phoneNumber" onChange={inputChange}
                            className="block w-full border border-gray-300 px-4 py-3 text-gray-600 text-sm rounded focus:ring-0 focus:border-primary placeholder-gray-400"
                            placeholder="Your Phone Number"/>
                    </div>
                    <div>
                        <label for="address" className="text-gray-600 mb-2 block">address</label>
                        <input type="address" name="address" id="address" onChange={inputChange}
                            className="block w-full border border-gray-300 px-4 py-3 text-gray-600 text-sm rounded focus:ring-0 focus:border-primary placeholder-gray-400"
                ...
                <div className="mt-6">
                    <div className="flex items-center">
                        <input type="checkbox" name="aggrement" id="aggrement" onChange={inputChange}
            ...
            <p className="mt-4 text-center text-gray-600">Already have account? <a href="login.html"
                    className="text-primary"><button onClick={onClickLogin}>Login now</button></a></p>
        </div>
    </div>
    </>
  )
